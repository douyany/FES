...
mydata$ylabel_vector[mydata$variable=="FFSIncomplete"] <- (-.5)*mydata$maxofrowsCompl
mydata$ylabel_vector[mydata$variable=="BLIncomplete"] <- (-1)*mydata$maxofrowsCompl
mydata$ylabel_vector[mydata$variable=="FLIncomplete"] <- (-1.5)*mydata$maxofrowsCompl

mydata$ylabel_vector[mydata$variable=="SurvIncomplete"] <- (-2)*mydata$maxofrowsCompl
...
xyplot(mydata$value ~ mydata$yrmo |factor(mydata$COUNTY_ID),
 ...
 ##saves file as pdf
pdf(file="h://mbg_db_reporting//reports//20150402_FESbymo_with_issue_records.pdf", paper="letter", height = 10) 
